# 1 "C:/Users/boto7/AppData/Roaming/Xilinx/Vivado/Webmodel_final/solution1/.autopilot/db/WebModel.pragma.1.c"
# 1 "C:/Users/boto7/AppData/Roaming/Xilinx/Vivado/Webmodel_final/solution1/.autopilot/db/WebModel.pragma.1.c" 1
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 147 "<built-in>" 3
# 1 "<command line>" 1
...
# 8 "<command line>" 2
# 1 "<built-in>" 2
# 1 "C:/Users/boto7/AppData/Roaming/Xilinx/Vivado/Webmodel_final/solution1/.autopilot/db/WebModel.pragma.1.c" 2
# 1 "../../../../Downloads/WebModel.c"
# 1 "../../../../Downloads/WebModel.c" 1
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 147 "<built-in>" 3
# 1 "<command line>" 1
...
# 3 "../../../../Downloads/WebModel.c" 2
# 1 "../../../../Downloads/WebModel.h" 1
...
# 4 "../../../../Downloads/WebModel.c" 2

void k2c_dense(k2c_tensor* output, const k2c_tensor* input, const k2c_tensor* kernel, const k2c_tensor* bias, int activation, float * fwork) {

 const size_t ndim_input = input->ndim;
    if (ndim_input <=2) {
     const size_t outrows = (ndim_input > 1) ? input->shape[0] : 1;
        ...
        const size_t axesA[1] = {ndim_input-1};
_ssdm_SpecConstant(axesA);
# 21 "../../../../Downloads/WebModel.c"

        const size_t axesB[1] = {0};
_ssdm_SpecConstant(axesB);
# 22 "../../../../Downloads/WebModel.c"

        const size_t naxes = 1;


        k2c_dot(output, input, kernel, axesA, axesB, naxes, 0, fwork);
...
void k2c_affine_matmul(float * C, const float * A, const float * B, const float * d, const size_t outrows,const size_t outcols, const size_t innerdim) {
  ...
  const size_t outrows_outcols = outrows * outcols;

     for (size_t p = 0; p < outrows_outcols; ++p) {
         C[p] = 0;
     }
...
void k2c_dot(k2c_tensor* C, const k2c_tensor* A, const k2c_tensor* B, const size_t * axesA, const size_t * axesB, const size_t naxes, const int normalize, float * fwork) {
    ...
    size_t i;
    size_t j;
#pragma omp parallel for reduction(*:free_axesA) private(i, j)
 for (i=0; i<ndimA; ++i) {
        isin = 0;
        for (j=0; j<naxes; ++j) {
_ssdm_op_SpecPipeline(-1, 1, 1, 0, "");
 if (i==axesA[j]) {
    ...
    count=0;
#pragma omp parallel for reduction(*:free_axesB) private(i, j)
 for (i=0; i<ndimB; ++i) {
        isin = 0;
        for (j=0; j<naxes; ++j) {
_ssdm_op_SpecPipeline(-1, 1, 1, 0, "");
 ...
 Asub[j] = Bsub[permB[j]];
    ...
    if (normalize) {
        ...
        for (i=0; i<free_axesA; ++i) {
            ...
            sum = 0;
            product =i*prod_axesA;
            for (j=0; j<prod_axesA; ++j) {
_ssdm_op_SpecPipeline(-1, 1, 1, 0, "");
 dot_reshapeA = reshapeA[product + j];
                sum += dot_reshapeA*dot_reshapeA;
            }
            inorm = 1.0f/sqrtf(sum);
            for (j=0; j<prod_axesA; ++j) {
_ssdm_op_SpecPipeline(-1, 1, 1, 0, "");
 reshapeA[product + j] *= inorm;
            }
        }
        size_t dot_reshapeB;
...
void k2c_bias_add(k2c_tensor* A, const k2c_tensor* b) {
  ...
  const size_t numel_A = A->numel;
     const size_t numel_b = b->numel;
     for (size_t i = 0; i < numel_A; i += numel_b) {
         for (size_t j = 0; j < numel_b; ++j) {
_ssdm_op_SpecPipeline(-1, 1, 1, 0, "");
...
void k2c_matmul(float * C, const float * A, const float * B, const size_t outrows, const size_t outcols, const size_t innerdim) {
 ...
 const size_t for_a = outrows * outcols;
     for (size_t a10 = 0; a10 < for_a; a10++) {
         C[a10] = 0;
     }
     size_t inner_k_outcols=0;
     size_t inner_k_outcols_end=0;
     size_t outrowidx=0;
     size_t inneridx=0;
 for (size_t k = 0; k < innerdim; ++k) {
         ...
         for (size_t i = 0; i < outrows; ++i) {
             ...
             for (size_t j = inner_k_outcols; j < inner_k_outcols_end; ++j) {
_ssdm_op_SpecPipeline(-1, 1, 1, 0, "");
 C[outrowidx + j - inner_k_outcols] += A[inneridx + k] * B[j];
             }
         }
     }
 }
...
static float dense_69_kernel_array[25536] = {
  -3.98242176e-01f,
  +7.93371499e-01f,
  +2.68897206e-01f,
  -4.02115196e-01f,
...
static float dense_69_fwork[25935] = {0};
...
_ssdm_op_SpecPipeline(-1, 1, 1, 0, "");
 dense_70_output.array[b4]=dense_70_output_array[b4];
  dense_70_bias.array[b4]=dense_70_bias_array[b4];
  dense_71_kernel.array[b4]=dense_71_kernel_array[b4];
  dense_69_bias.array[b4]=dense_69_bias_array[b4];
  dense_69_output.array[b4]=dense_69_output_array[b4];
  }
...
_ssdm_SpecArrayPartition( &dense_69_kernel.array, 1, "CYCLIC", 4, "");
...
_ssdm_op_SpecPipeline(-1, 1, 1, 0, "");
 ...
 dense_71_bias.array[0]=dense_71_bias_array[0];
